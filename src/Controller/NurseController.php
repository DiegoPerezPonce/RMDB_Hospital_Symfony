...
final class NurseController extends AbstractController
{
    ...
    public function login(Request $request): JsonResponse
    {
        ...
        #[Route('/nurse/{id}', name: 'nurse_delete', methods: ['DELETE'])]
    public function delete(int $id): JsonResponse
    {
        $nurse = $this->em->getRepository(Nurse::class)->find($id);

        if (!$nurse) {
            return $this->json(['success' => false, 'message' => 'Nurse not found.'], Response::HTTP_NOT_FOUND);
        }

        $this->em->remove($nurse);
        $this->em->flush();

        return $this->json(['success' => true, 'message' => 'Nurse deleted successfully.'], Response::HTTP_OK);
    }
}
